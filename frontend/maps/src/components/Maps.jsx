...
const vehicleIcon = new L.Icon({
    iconUrl: 'https://cdn-icons-png.flaticon.com/512/3448/3448857.png',
    iconSize: [40, 40],
    iconAnchor: [20, 40],
    popupAnchor: [0, -40]
});
...
const Maps = () => {
    ...
    return (
        <div style={{ position: 'relative', width: '100%', height: '100vh' }}>
            <div style={{ position: 'absolute', top: '10px', left: '50%', transform: 'translateX(-50%)', background: 'white', padding: '10px', borderRadius: '5px', boxShadow: '0 2px 5px rgba(0,0,0,0.2)', display: 'flex', alignItems: 'center', zIndex: 1000 }}>
                ...
                <input 
                    ...
                    style={{ border: 'none', outline: 'none', padding: '5px', flexGrow: 1 }}
                />
                {searchInput && <FaTimes style={{ marginLeft: '10px', cursor: 'pointer' }} onClick={() => setSearchInput('')} />}
            </div>
            <MapContainer center={userLocation} zoom={13} style={{ width: '100%', height: '100%' }}>
                <RecenterMap center={userLocation} />
                <TileLayer url="https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png" />
                <Marker position={userLocation} icon={vehicleIcon}><Popup>Your Location</Popup></Marker>
                {route.length > 0 && <Polyline positions={route} color="blue" />}
            </MapContainer>
            <button 
                style={{ position: 'absolute', bottom: '20px', right: '20px', background: 'white', borderRadius: '50%', padding: '10px', boxShadow: '0 2px 5px rgba(0,0,0,0.3)', cursor: 'pointer', zIndex: 1000 }}
                onClick={() => setUserLocation([...userLocation])}
            >
                <FaLocationArrow />
            </button>
            {distance && <div style={{ position: 'absolute', bottom: '60px', right: '20px', background: 'white', padding: '10px', borderRadius: '5px', boxShadow: '0 2px 5px rgba(0,0,0,0.3)', zIndex: 1000 }}>Distance: {distance} km</div>}
        </div>
    );
};
