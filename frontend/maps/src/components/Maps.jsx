...
const Maps = () => {
    ...
    // Geo-Fencing Coordinates (Example Area)
    const geoFenceArea = {
        latMin: 51.500,
        latMax: 51.510,
        lngMin: -0.100,
    ...
    useEffect(() => {
        if (navigator.geolocation) {
            navigator.geolocation.watchPosition((position) => {
                ...
                // Check if user is outside the geo-fence
                if (
                    latitude < geoFenceArea.latMin ||
                    latitude > geoFenceArea.latMax ||
                    longitude < geoFenceArea.lngMin ||
                    longitude > geoFenceArea.lngMax
                ) {
                    alert("Warning: You are leaving the designated area!");
                }
            }, (error) => {
                console.error("Error getting location:", error);
            });
